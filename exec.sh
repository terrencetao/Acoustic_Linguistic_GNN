...
for unit in $(seq 1500 1000 10000);do
for mma in fixed ;do
...
for alpha in $(seq 1.0 1.0 1.0);do
 #for n in $(seq 0.02 0.01 0.1);do #densite de 50%
   num=$(echo "0.50*($unit/8 - 1)" | bc | awk '{print int($0)}')
 ...
 for mhg in ML ;do
   for msw in phon_count;do #phon_suit phon_coo mixed semantics;do
	...
	num_n_h=$(echo "0.25*($unit/8 - 1)" | bc | awk '{print int($0)}')
	...
	python3 gnn_model.py --input_folder ''  --graph_file saved_graphs/$dataset/$mma/$msa/kws_graph_"$num"_"$unit".dgl --epochs 50
	python gnn_heto_model.py --input_folder '' --graph_file saved_graphs/$dataset/$mma/$msa/$mhg/hetero_graph_"$num_n_h"_"$unit".dgl --epochs 50

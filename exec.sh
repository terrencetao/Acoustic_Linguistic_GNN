...
######################################################### Heterogeneous graph and Heterogenous GNN #####################################################################################
 for mhg in  dnn  fixed  ;do
   for msw in phon_count ;do #phon_suit phon_coo mixed semantics;do
	...
	python3 heterogenous_graph.py --twa $twa --num_n $num_n_h --method $mhg --msw $msw --sub_units $unit --method_sim $mma --method_acou $msa --dataset $dataset --num_n_ac $num --k_out $ko # twa = treshold  for acoustic-linguistic probability, number link for each linguistic node

	                                            # path to hetero_graph: saved_graphs/dataset/$mma/$msa/$mhg/$msw/
	python3 gnn_heto_model.py --input_folder '' --graph_file saved_graphs/$dataset/$mma/$msa/$mhg/$msw/hetero_graph_"$num"_"$ko"_"$num_n_h"_"$unit".dgl --epochs 100 --lamb 1
...
############################################################# TEST #####################################################################################################
	python3 eval_embedding.py --mma $mma --twa $twa --num_n_h $num_n_h --mhg $mhg  --num_n_a $num --k_out $ko --ta 0.5 --alpha $alpha --tw 0.5  --msw $msw --msa $msa --mgw $mgw --sub_unit $unit --drop_freq 0.0 --drop_int 0.0 --dataset $dataset --lamb 1
	for add in dnn ;do
	python3 induct_eval_embedding.py --mma $mma --twa $twa --num_n_h $num_n_h --mhg $mhg  --num_n_a $num --k_out $ko --ta 0 --alpha $alpha --tw 0.5  --msw $msw --msa $msa --mgw $mgw --sub_unit $unit --drop_freq 0.0 --drop_int  0.0 --dataset $dataset --add $add --k_inf $num --lamb 1
	echo ""
        done
        done
        done

...
# 3. Passage dans le modèle
    ...
    # Top-k indices avec tri
       top_k_indices = np.argpartition(-similarities, k)[:k]
       top_k_indices = top_k_indices[np.argsort(-similarities[top_k_indices])]  # ordonné

    # Création des arêtes bidirectionnelles pondérées par la similarité
       edges = [(new_node_index, i, similarities[i]) for i in top_k_indices]
       edges += [(i, new_node_index, similarities[i]) for i in top_k_indices]
...
def add_new_acoustic_nodes_to_hetero_graph_knn(hetero_graph, new_node_spectrograms, k, distance_function, ml_model, ml_dense, threshold_probability, add, n_jobs=-1):
    ...
    ml_model.eval()
    new_node_features = torch.tensor(new_node_spectrograms)
    #new_node_features = new_node_features.unsqueeze(1)
# Predict softmax probabilities
    with torch.no_grad():  # Disable gradient calculation
         logits = ml_dense(new_node_features)
         ml_predictions = F.softmax(logits, dim=1)

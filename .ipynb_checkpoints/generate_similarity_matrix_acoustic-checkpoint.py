...
def sim_matrix(method, subset_labels=None, subset_spectrograms=None, word_matrix=None):
    ...
    if method == 'dtw':
        ...
        embeddings = torch.from_numpy(subset_spectrograms).float()
        # 1. Calcul de la matrice de distance euclidienne
        x_norm = (embeddings ** 2).sum(1).unsqueeze(1)
        dist_matrix = torch.sqrt(x_norm + x_norm.T - 2.0 * (embeddings @ embeddings.T))
        dist_matrix = torch.nan_to_num(dist_matrix, nan=0.0)  # gérer les NaN éventuels
        sim_matrix_from_distance = 1 / (1 + dist_matrix)
        # 3. Remplacement des zéros dans similarity_matrix
        similarity_matrix = torch.from_numpy(similarity_matrix).float()
        zero_mask = (similarity_matrix == 0)
        similarity_matrix[zero_mask] = sim_matrix_from_distance[zero_mask]

        # 4. (Optionnel) Rendre symétrique
        ...
        #            similarity_matrix[i, j] = F.pairwise_distance(embeddings[i],embeddings[j])
        #            similarity_matrix[j, i] = F.pairwise_distance(embeddings[i],embeddings[j])
...
if __name__ == "__main__":
	...
	if not os.path.isfile(similarity_matrix_path):
		...
		(train_spec_induc_val, train_labels_induc_val), (train_spectrograms_filter, labels_train_filter) = filter_pairs_by_label_dict(train_spectrograms, labels_train, induc_val_labels_inv)
                
		(val_spec_induc_val, val_labels_induc_val), (val_spectrograms_filter, labels_val_filter) = filter_pairs_by_label_dict(train_spectrograms, labels_train, induc_val_labels_inv)
		
		# reencode les labels pour besion de linearite
		encoder = LabelEncoder()
		labels_train_reencoded = encoder.fit_transform(labels_train_filter)
		
		print(f'train-spec shape after filtration: {len(train_spectrograms_filter)}')
        
		
		with open(f'subset_label_names_{args.dataset}.pkl', 'rb') as f:
			label_dic_names = pickle.load(f)
		label_word_names = label_dic_names.keys()  

		with open(f'label_to_index_{args.dataset}.pkl', 'rb') as f:
			label_to_original_index = pickle.load(f)
            
        	# Récupérer tous les indices (valeurs) à encoder
		indices_to_encode = {label_to_original_index[label] for label in label_word_names}
                                                          
        	# Récupérer les classes connues par l'encoder (doivent être des entiers ici)
		known_indices = set(encoder.classes_)

        	# Vérifier que tous les indices sont encodables
		missing_indices = indices_to_encode - known_indices

		if missing_indices:
			print("❌ Les indices suivants ne sont pas encodables :", missing_indices)
		else:
			print("✅ Tous les indices sont encodables par le LabelEncoder.")
		...
		labels_val_reencoded = encoder.transform(labels_val_filter)
		...
		subset_spectrograms, subset_labels = stratified_sample(train_spectrograms_filter, labels_train_reencoded, math.floor(subset_size*0.8))
		print(len(subset_labels))
		subset_val_spectrograms, subset_val_labels = stratified_sample(val_spectrograms_filter, labels_val_reencoded, subset_size-math.floor(subset_size*0.8))

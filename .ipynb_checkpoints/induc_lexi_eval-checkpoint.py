...
def add_new_nodes_to_hetero_graph_knn(hetero_graph, new_node_spectrograms, n_jobs=-1):
    ...
    def process_new_node(new_node_index):
        ...
        new_embedding = flattened_new_features[rel_idx].unsqueeze(0)  # (1, D)

        # Calcul pairwise des distances euclidiennes
        distances = F.pairwise_distance(new_embedding, existing_features)  # (N,)

        # Conversion des distances en similarités
        similarities = 1 / (1 + distances)  # (N,) → plus la distance est faible, plus la similarité est grande

        # Création d'arêtes bidirectionnelles avec poids
        edges = [(new_node_index, i, similarities[i].item()) for i in range(num_existing_nodes)]
        edges += [(i, new_node_index, similarities[i].item()) for i in range(num_existing_nodes)]
...
# Ajuster la dimension des nouvelles features
    ...
    # Padding des nouvelles features
       ...
       flattened_new_features = flattened_acoustic_features
...
def evaluate_acoustic_word_link_prediction_topk(gcn_model, hetero_graph,
    ...
    with torch.no_grad():
        ...
        for i in range(num_new):
            ...
            predicted_word_labels =  [hetero_graph.nodes['word'].data['label'][idx] for idx in topk_indices[i]]
...
def evaluate_link_prediction_classification_topk(
    ...
    with torch.no_grad():
        ...
        labels_acoustic_original = encoder.inverse_transform(labels_acoustic)
        for i in range(num_new):
            true_label = index_to_original_label[labels_acoustic_original[i]]
            
            predicted_word_labels = [label_names[idx] for idx in topk_indices[i]]

...
#################################### CONFIGURATION #################################################
DATASETS=("google_commands" "mini_google_commands"  "spoken_digit" "yemba_command_small")
UNITS=$(seq 3000 100 8000)
METHOD_MMA="clique"
METHOD_MSA="filter"
ALPHAS=(1.0)
...
build_heterogeneous_graph_and_eval() {
  ...
  for mhg in "${MHG_METHODS[@]}"; do
        ...
        python3 gnn_heto_link_pred_model.py --input_folder '' --graph_file "$graph_file" --epochs 1000 --lamb $lamb --dataset $dataset\
            ...
            python3 gnn_heto_model.py --input_folder '' --graph_file "$graph_file" --epochs 1 --lamb $lamb \

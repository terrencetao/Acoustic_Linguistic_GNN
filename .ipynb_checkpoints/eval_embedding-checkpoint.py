...
def get_idx_from_label(label, hetero_graph):
    labels = hetero_graph.node['word'].data['label']
...
def evaluate_acoustic_word_link_prediction_topk(gcn_model, hetero_graph,
    ...
    with torch.no_grad():
        ...
        for i in range(num_emb_a):
            true_label = labels_acoustic[i]
            pred_labels_topk = [hetero_graph.nodes['word'].data['label'][idx] for idx in topk_indices[i]]
...
def evaluate_with_true_graph_adjacency(gcn_model, hetero_graph, top_k=5):
    """
    Évalue la précision Top-1 et Top-K en comparant les prédictions avec les vraies arêtes du graphe.

    Paramètres :
    - gcn_model : Modèle GNN entraîné.
    - hetero_graph : Graphe hétérogène avec les arêtes réelles.
    - top_k : Valeur de K pour la précision Top-K.

    Retourne :
    - accuracy_top1 : Précision Top-1.
    - accuracy_topk : Précision Top-K.
    """
    gcn_model.eval()
    device = next(gcn_model.parameters()).device
    ...
    with torch.no_grad():
        ...
        # Calcul des scores de lien
        edge_features = []
        for i in range(num_emb_a):
            e_a = emb_acoustic[i].unsqueeze(0).repeat(emb_word.size(0), 1)
            e_w = emb_word
            diff = torch.abs(e_w - e_a)
            prod = e_w * e_a
            feat = torch.cat([e_w, e_a, diff, prod], dim=1)
            edge_features.append(feat)

        edge_features = torch.stack(edge_features)  # [num_acoustic, num_word, 4d]
        pred_scores = gcn_model.edge_predictor(edge_features).squeeze(-1)  # [num_acoustic, num_word]

        # Prédictions top-k
        topk_indices = torch.topk(pred_scores, k=top_k, dim=1).indices.cpu().numpy()

        # Construction de la ground truth à partir du graphe
        true_edges = hetero_graph.edges(etype=('word', 'related_to', 'acoustic'))
        acoustic_src = true_edges[1].cpu().numpy()
        word_dst = true_edges[0].cpu().numpy()

        acoustic_to_word = {i: [] for i in range(num_emb_a)}
        for a, w in zip(acoustic_src, word_dst):
            acoustic_to_word[a].append(w)
        ...
        for i in range(num_emb_a):
            true_words = acoustic_to_word.get(i, [])
            preds = topk_indices[i]
            if true_words:
                if preds[0] in true_words:
                    correct_top1 += 1
                if any(p in true_words for p in preds):
                    correct_topk += 1

        total = sum(1 for i in range(num_emb_a) if acoustic_to_word.get(i))
...
def evaluate_acoustic_word_link_prediction_topk2(gcn_model, hetero_graph,
                                                labels_acoustic,
                                                top_k=5):
    """
    Predict links and compute top-1 and top-K accuracy between new acoustic and word nodes.
    ...
    with torch.no_grad():
        ...
        scores = torch.matmul(emb_acoustic, emb_word.T)

        pred_scores = torch.sigmoid(scores)  # [num_emb_a, num_word]
        ...
        topk_indices = topk_indices_pred # l'on revient a l'encodage originel celui contenu dans le graph word, pour pouvoir comparer 
        correct_top1 = 0
        correct_topk = 0
        true_labels = labels_acoustic
...
acc_pred_link, acc_topk  =evaluate_with_true_graph_adjacency(

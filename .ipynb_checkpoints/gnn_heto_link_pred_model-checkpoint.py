...
def train_link_regression(model, g, features, true_edge_labels,
                          src, dst, src_pos, dst_pos, src_neg, dst_neg,
                          val_ratio=0.2, epochs=100, lr=0.0001, lamb=0.0, alpha=0.0):
    ...
    split_idx = int((1 - val_ratio) * num_edges)
...
def main(input_folder, graph_file, epochs, lamb, dataset):
    ...
    model = train_link_regression(
    model, hetero_graph, features, true_edge_labels,
    src, dst,
    src_pos, dst_pos, src_neg, dst_neg,
    epochs=epochs, lr=0.0001, lamb=0.1, alpha=0.05  # adjust lamb/alpha as needed
)

      ...
      for mgw in "${MGW_METHODS[@]}"; do
        ...
        for twa in "${TWAS[@]}"; do
          for p in $(seq "${PS[0]}" 2 "${PS[1]}"); do
            ...
            python3 gnn_heto_model.py --input_folder '' --graph_file "$graph_file" --epochs 100 --lamb 1 \
            ...
            for add in dnn; do
              python3 induct_eval_embedding.py --mma "$mma" --twa "$twa" --num_n_h "$num_n_h" --mhg "$mhg" \
                ...
                --msa "$msa" --mgw "$mgw" --sub_unit "$unit" --drop_freq 0.0 --drop_int 0.0 \
                --dataset "$dataset" --add "$add" --k_inf "$num" --lamb $lamb --densite $p
            done
          done
        done
      done
